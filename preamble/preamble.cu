...
__device__ inline float2 readPixel(int width, int height, int depth, float* imageData, int sampler, int4 position)
{ 
    int4 pos = make_int4(position.x, position.y, position.z, 0);
...
__device__ inline void writePixel(float* imageData, int width, int height, int depth, int4 pos, float pixelValue)
{ 
    int index = pos.z * width * height + pos.y * width + pos.x; 
